circuitscapeFct = function(envVariable, envVariableName, resistance=TRUE, avgResistance=TRUE, fourCells=FALSE, fromCoor, toCoor, OS="Unix", prefix="", ID="", nberOfCores_CS=1)	{
	...
	colnames(lines1) = NULL; lines2 = as.matrix(lines2); colnames(lines2) = NULL
	...
	if (OS == "Unix")
		{
			system(paste("python2.7",paste(folder,"CS_script.sh", sep="/"),sep=" "), ignore.stdout=F, ignore.stderr=F)
